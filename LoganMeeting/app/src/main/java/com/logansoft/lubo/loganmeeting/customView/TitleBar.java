...
public class TitleBar extends ViewGroup implements View.OnClickListener {
    ...
    public void setLeftText(CharSequence title) {
        ...
        //改
        mLeftText.setGravity(Gravity.CENTER_VERTICAL);
        mLeftText.setPadding(mLeftText.getPaddingLeft(), mLeftText.getPaddingTop() + 58,
                mLeftText.getPaddingRight(), mLeftText.getPaddingBottom());
        if (mLeftText.getText() != null) {
            img = getResources().getDrawable(R.mipmap.nav_btn_back);
    ...
    public void setTitle(CharSequence title) {
        ...
        if (index > 0) {
            ...
            if (index > 0) {
                setTitle(title.subSequence(0, index), "  " + title.subSequence(index + 1, title
                        .length()), LinearLayout.HORIZONTAL);
            } else {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mLeftText.getMeasuredWidth() > mRightLayout.getMeasuredWidth()) {
            mCenterLayout.measure(MeasureSpec.makeMeasureSpec(mScreenWidth - 2 * mLeftText
                    .getMeasuredWidth(), MeasureSpec.EXACTLY), heightMeasureSpec);
        } else {
            mCenterLayout.measure(MeasureSpec.makeMeasureSpec(mScreenWidth - 2 * mRightLayout
                    .getMeasuredWidth(), MeasureSpec.EXACTLY), heightMeasureSpec);
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        mLeftText.layout(0, mStatusBarHeight, mLeftText.getMeasuredWidth(), mLeftText
                .getMeasuredHeight() + mStatusBarHeight);
        ...
        if (mLeftText.getMeasuredWidth() > mRightLayout.getMeasuredWidth()) {
            mCenterLayout.layout(mLeftText.getMeasuredWidth(), mStatusBarHeight, mScreenWidth -
                    mLeftText.getMeasuredWidth(), getMeasuredHeight());
        } else {
            mCenterLayout.layout(mRightLayout.getMeasuredWidth(), mStatusBarHeight, mScreenWidth
                    - mRightLayout.getMeasuredWidth(), getMeasuredHeight());
        }
